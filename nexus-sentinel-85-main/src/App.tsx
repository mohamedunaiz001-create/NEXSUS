...
export default function App() {
  ...
  const [threatScore, setThreatScore] = useState<number>(() => {
    const saved = localStorage.getItem('cyber_threat_score');
    return saved ? Number(saved) : 78;
  });
  const [threatThreshold, setThreatThreshold] = useState<number>(() => {
    const saved = localStorage.getItem('cyber_threat_threshold');
  ...
  useEffect(() => {
    localStorage.setItem('cyber_threat_score', String(threatScore));
  }, [threatScore]);

  useEffect(() => {
    localStorage.setItem('cyber_threat_threshold', String(threatThreshold));
  }, [threatThreshold]);
  ...
  const [ceo, setCeo] = useState<CEONode>(() => {
    try {
      const saved = localStorage.getItem('cyber_ceo_state');
      if (saved) {
  ...
  const [providers, setProviders] = useState<AIProvider[]>(() => {
    try {
      const saved = localStorage.getItem('cyber_ai_providers');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
          const sanitized = stripApiKeys(parsed);
          localStorage.setItem('cyber_ai_providers', JSON.stringify(sanitized));
          return sanitized;
        }
      }
    } catch (e) {
  ...
  const handleUpdateCeo = (updates: Partial<CEONode>) => {
    setCeo(prev => {
      ...
      try {
        localStorage.setItem('cyber_ceo_state', JSON.stringify(updated));
      } catch (e) {
