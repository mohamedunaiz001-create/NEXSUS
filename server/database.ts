...
export function upsertCase(input: Omit<DbCase, 'createdAt' | 'updatedAt'>, createdAt?: string): DbCase {
  ...
  `).run(input.id, input.title, input.severity, input.stage, input.summary, input.assignedAgent, input.confidence, input.ownerId, created, now);
...
export function createSession(jti: string, userId: string, expiresAt: string): void {
  db.prepare('INSERT INTO sessions (jti,user_id,created_at,expires_at) VALUES (?,?,?,?,?)')
...
export function revokeSession(jti: string): void {
  db.prepare('UPDATE sessions SET revoked_at = ? WHERE jti = ? AND revoked_at IS NULL')
